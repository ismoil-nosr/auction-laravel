...
@section('content')
    <form class="form form--add-lot container {{ count($errors) ? 'form--invalid' : ''}}" action="/add-lot" method="post" enctype="multipart/form-data">
        @csrf
        <h2>Добавление лота</h2>
        <div class="form__container-two">
            <div class="form__item @error('name') form__item--invalid @enderror">
                <label for="lot-name">Наименование</label>
                <input id="lot-name" type="text" name="name" placeholder="Введите наименование лота" value="{{ old('name') }}">
        ...
        <div class="form__item form__item--file @error('img') form__item--invalid @enderror {{ old('img') ? 'form__item--uploaded' : ''}}">
